              ...
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 xl:grid-cols-4">
                {filteredArticles.map((artikel, index) => (
                  <Link
                    ...
                    className="group flex flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white transition-all hover:shadow-xl hover:-translate-y-1"
                  >
                    <article className="flex flex-col h-full">
                      ...
                      <div className="flex flex-1 flex-col p-5">
                        ...
                        <h3 className="mb-3 text-base font-extrabold leading-tight text-[#17391f] line-clamp-2 transition-colors group-hover:text-primary min-h-[3rem]">
                          {artikel.title}
                        </h3>

                        <p className="mb-5 line-clamp-2 text-xs font-medium text-[#5d7a64] leading-relaxed">
                          {artikel.description}
                        </p>
                ...
                <div className="flex flex-col gap-3">
                  {ARTICLE_POPULAR_CATEGORIES.map((cat, idx) => (
                    <button
                      ...
                      className="group flex items-center justify-between py-1 transition-all text-left cursor-pointer"
                    >
                      <span className="text-sm font-bold text-[#5d7a64] transition-colors group-hover:text-primary">
                        {cat.label}
                      </span>
                      <span className="text-xs font-extrabold text-[#17391f] bg-white px-2 py-0.5 rounded-lg border border-slate-100 shadow-sm">
                        {cat.jumlahArtikel}
                      </span>
                    </button>
                  ))}
                  <button onClick={handleReset} className="mt-6 flex items-center gap-1.5 text-xs font-extrabold text-primary hover:underline cursor-pointer text-left uppercase tracking-wider">
              ...
              <div className="px-2">
                ...
                <div className="flex flex-col gap-6">
                  {ARTICLE_MOST_POPULAR.map((trending, idx) => (
                    ...
                    >
                      <span className="flex size-7 shrink-0 items-center justify-center rounded-lg bg-primary/5 text-xs font-extrabold text-primary">
                        {trending.rank}
                      </span>
                      <div className="flex gap-4">
                        <div className="relative size-16 shrink-0 overflow-hidden rounded-xl bg-slate-100 border border-slate-50 shadow-sm">
                          <Image
