    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl md:text-2xl font-extrabold text-[#17391f]">
            {INVESTMENT_PORTFOLIO_DUMMY.heading}
          </h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 rounded-2xl bg-[#EAF3DE]/60 border border-[#d4edda] shadow-sm">
          <p className="text-[10px] font-semibold uppercase tracking-wider text-[#5d7a64] mb-1">
            Total Investasi
          </p>
              ...
              {investasiAktif.map((item, index) => (
                <tr key={index} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <Badge
                      variant="outline"
                      className="bg-[#EAF3DE] text-primary border-none text-[10px] font-bold h-5"
                    >
                      {item.status}
                    </Badge>
                  </td>
