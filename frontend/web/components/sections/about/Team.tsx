    ...
    <section className="section-padding">
      <div className="container-smarttani">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 lg:gap-12">
            ...
            <div className="space-y-4">
              {ABOUT_ACHIEVEMENTS.items.map((item, index) => (
                <div key={index} className="flex items-start gap-3">
                  ...
                  <span className="text-sm font-medium text-[#5d7a64] leading-relaxed">{item}</span>
                </div>
              ))}
            </div>
          </div>
              ...
              {ABOUT_LEADERSHIP.items.map((pimpinan) => (
                <div key={pimpinan.nama} className="flex items-center gap-4 group">
                  ...
                  <div>
                    ...
                    <p className="text-xs font-medium text-[#5d7a64]">{pimpinan.jabatan}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Alamat & Kontak */}
          <div>
            <h3 className="text-lg font-extrabold text-[#17391f] mb-6">{ABOUT_ADDRESS.heading}</h3>
            <div className="space-y-5">
