...
import { ArrowRight, Star, Quote, Compass, GraduationCap, Medal } from 'lucide-react';
...
const InfoGrid = () => {
  ...
  const jalurBgs = ['bg-primary-light', 'bg-blue-50', 'bg-primary-light'];
            ...
            <div className="flex flex-col gap-2.5">
              {ACADEMY_LEARNING_PATHS.items.map((jalur, index) => (
                <React.Fragment key={jalur.level}>
                  <div
                    className={`${jalurBgs[index]} rounded-xl p-3.5 flex items-center gap-3 border border-gray-100 shadow-sm transition-all hover:border-gray-300`}
                  >
                    <div className="shrink-0 flex size-10 items-center justify-center rounded-xl bg-white shadow-sm text-primary">
                      {jalurIcons[index]}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="text-sm font-bold text-[#17391f] leading-tight">
                        {jalur.level}
                      </h4>
                    ...
                    <div className="shrink-0 text-right">
                      ...
                      <p className="text-[10px] font-medium text-[#5d7a64] whitespace-nowrap">
                        {jalur.jumlahJam} Jam
                      </p>
                    </div>
                  </div>
          ...
          <div className="md:col-span-1">
            ...
            <div className="flex flex-col gap-4">
              {ACADEMY_INSTRUCTORS.items.slice(0, 2).map((instruktur) => (
                <div
                  key={instruktur.nama}
                  className="flex items-center gap-4 bg-[#EAF3DE] rounded-2xl p-4 shadow-sm border border-transparent transition-all hover:border-gray-300"
                >
                  ...
                  <div className="min-w-0">
                    <h4 className="text-sm font-extrabold text-[#17391f] truncate">
                      {instruktur.nama}
                    </h4>
                    <p className="text-xs font-medium text-[#5d7a64] mb-1.5 truncate">
                      {instruktur.gelar.split(' di ')[0]}
                    </p>
                    <div className="flex items-center gap-1.5">
                      <Star className="size-3.5 fill-accent text-accent" />
                      <span className="text-xs font-bold text-[#17391f]">{instruktur.rating}</span>
                      <span className="text-xs font-medium text-[#5d7a64]">
                        ({instruktur.jumlahPeserta.toLocaleString()} peserta)
                      </span>
                    </div>
                  </div>
                </div>
            ...
            <div className="flex flex-col rounded-2xl bg-[#EAF3DE] p-6 shadow-sm min-h-[160px]">
              <div className="mb-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[0.65rem] font-medium text-[#5d7a64] md:text-xs truncate">
                      {testimonial.role}
                    </p>
                  </div>
                </div>
              ...
              <p className="text-xs leading-relaxed italic text-[#5d7a64] md:text-sm">
                ...
                {testimonial.quote.length > 140
                  ? testimonial.quote.substring(0, 140) + '...'
                  : testimonial.quote}
                &rdquo;
              </p>
            </div>
