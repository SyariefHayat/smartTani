      ...
      <div className="container-smarttani">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3 lg:grid-cols-4">
          {ACADEMY_KURSUS.items.map((kursus) => {
            const isOrangeBadge =
              kursus.badge === 'BEST SELLER' || kursus.badge === 'BARU' || kursus.badge === 'HOT';
            ...
            return (
              ...
              >
                ...
                <div className="relative aspect-[16/10] w-full overflow-hidden">
                  <Image src={kursus.image} alt={kursus.title} fill className="object-cover" />

                  {kursus.badge && (
                    <Badge
                      className={`absolute left-3 top-3 px-2 py-0.5 text-[10px] font-bold border-none shadow-sm rounded !text-white ${
                        isOrangeBadge ? 'bg-accent' : 'bg-primary'
                      }`}
                    >
                      {kursus.badge}
                    </Badge>
                ...
                <div className="flex flex-col flex-1 p-5">
                  ...
                  <div className="mb-5 flex items-center gap-2 text-xs font-bold">
                    ...
                    <span className="text-[#5d7a64] font-medium">
                      ({kursus.ulasan.toLocaleString('id-ID')})
                    </span>
                  </div>
                  ...
                  <div className="mt-auto mb-5 flex items-center justify-between gap-2">
                    ...
                    <span className="text-caption font-bold text-[#5d7a64]">{kursus.durasi}</span>
                  </div>

                  {/* Button */}
                  <Button
                    asChild
                    className="h-10 w-full bg-primary text-sm font-bold !text-white hover:bg-primary-dark rounded-xl transition-all shadow-sm cursor-pointer"
                  >
