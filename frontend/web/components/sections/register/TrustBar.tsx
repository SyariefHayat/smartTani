        ...
        <div className="grid grid-cols-2 gap-8 md:grid-cols-4 lg:gap-12">
          {REGISTER_TRUST_BAR.map((item, index) => {
            ...
            return (
              <div
                key={item.title}
                className="flex flex-col items-center text-center md:flex-row md:text-left md:gap-4"
              >
                ...
                <div>
                  <h3 className="text-body font-bold text-gray-900">{item.title}</h3>
                  <p className="mt-1 text-caption font-medium text-gray-500">{item.description}</p>
                </div>
              </div>
            );
          })}
