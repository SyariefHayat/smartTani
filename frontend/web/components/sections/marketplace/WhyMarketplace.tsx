...
import { ShieldCheck, ShoppingCart, Lock, Truck, Headphones } from 'lucide-react';
    ...
    <section className="container-smarttani mb-8">
      <div className="bg-[#E6E6E6] rounded-lg p-8">
        ...
        <div className="text-center mb-10 md:mb-14">
          <h2 className="text-xl md:text-2xl lg:text-3xl font-bold text-[#1a3a22]">{heading}</h2>
        </div>
          ...
          {items.map((item, index) => {
            ...
            return (
              ...
              >
                ...
                {/* Text Content */}
                <h3 className="text-sm md:text-base font-bold text-[#1a3a22] mb-2">{item.title}</h3>
                ...
                {!isLast && (
                  <div className="hidden lg:block absolute -right-[0.5px] top-1/2 -translate-y-1/2 h-20">
                    <Separator orientation="vertical" className="bg-[#d1d5d1]" />
                  </div>
                )}
              </div>
            );
