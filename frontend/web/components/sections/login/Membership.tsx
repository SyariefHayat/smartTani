  ...
  return (
    <div className="space-y-6">
      <div className="text-center lg:text-left">
        <h2 className="text-xl font-extrabold text-white mb-1">{LOGIN_MEMBERSHIP.title}</h2>
        <p className="text-sm font-medium text-white/70">{LOGIN_MEMBERSHIP.subtitle}</p>
      </div>
        ...
        {LOGIN_MEMBERSHIP.items.map((item) => (
          <div key={item.id} className="h-full">
            ...
            >
              ...
              >
                {selectedRole === item.id && <div className="size-1.5 rounded-full bg-primary" />}
              </div>
            </Label>
          </div>
        ))}
