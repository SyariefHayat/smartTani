    ...
    const handleStorage = () => {
      ...
      if (cart) {
        try {
          ...
          if (Array.isArray(parsed)) setCartCount(parsed.reduce((s, i) => s + (i.qty || 1), 0));
        } catch {
          setCartCount(0);
        }
      } else setCartCount(0);
            ...
            <Link href="/" id="navbar-logo" className="flex items-center gap-3 shrink-0">
              ...
              <div className="hidden lg:block">
                ...
                <p className="text-xs text-muted-foreground leading-tight max-w-70">
                  Platform terintegrasi untuk ekosistem pertanian yang lebih cerdas, produktif, dan
                  berkelanjutan.
                </p>
              </div>
            </Link>
              ...
              {user ? (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="ghost"
                      className="gap-2 px-2 cursor-pointer focus-visible:ring-0"
                    >
              ...
              {user ? (
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button
                      variant="ghost"
                      size="icon"
                      className="cursor-pointer h-9 w-9 focus-visible:ring-0"
                    >
                ...
                <div className="flex items-center gap-1.5">
                  <Button
                    variant="ghost"
                    size="sm"
                    className="gap-1 cursor-pointer h-8 px-3 text-xs font-medium text-white hover:bg-white/20 hover:text-white"
                    asChild
                  >
                    <Link href="/login">
