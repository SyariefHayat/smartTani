...
interface InstructorNotification extends Omit<NotificationItem, 'type'> {
  type: 'enrollment' | 'review' | 'publish' | 'system';
}

const MOCK_NOTIFICATIONS: InstructorNotification[] = [
  {
    id: 'notif-1',
    user_id: 'I-01',
    type: 'enrollment',
    title: 'Murid Baru Mendaftar! 👥',
    message: 'Farhan Setiawan mendaftar di kelas "Budidaya Hidroponik Modern untuk Pemula".',
    is_read: false,
  ...
  },
  {
    ...
    is_read: false,
    created_at: new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString(), // Today
  },
  {
    id: 'notif-3',
    user_id: 'I-01',
  ...
  },
  {
    ...
    user_id: 'I-01',
    type: 'system',
    title: 'Pembaruan Kebijakan Akademia',
    message:
      'SmartTani Academy merilis panduan kurikulum 2026. Silakan cek dasbor pengajar untuk detail perubahan.',
    is_read: true,
    created_at: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString(), // Older
  },
];

type NotificationType = 'all' | 'enrollment' | 'review' | 'publish' | 'system';
...
export default function InstructorNotificationsPage() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useQuery<InstructorNotification[]>({
    queryKey: ['instructor-notifications', user?.id],
    queryFn: async () => {
      const res = await notificationService.getNotifications();
      return res as unknown as InstructorNotification[];
    },
  });
  ...
  // Client-side state for snappy optimistic actions
  const [localNotifications, setLocalNotifications] = React.useState<InstructorNotification[]>([]);
  ...
  const readMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['instructor-notifications', user?.id] });
    },
  });
  ...
  const groupedNotifications = React.useMemo(() => {
    const todayGroup: InstructorNotification[] = [];
    const yesterdayGroup: InstructorNotification[] = [];
    const olderGroup: InstructorNotification[] = [];
  ...
  const renderIcon = (type: string) => {
    switch (type) {
      ...
      case 'review':
        return (
          <div className="h-9 w-9 rounded-lg bg-amber-50 border border-amber-100 flex items-center justify-center text-amber-500 shrink-0 shadow-xs">
      ...
      default:
        return (
          <div className="h-9 w-9 rounded-lg bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-500 shrink-0 shadow-xs">
  ...
  return (
    <div className="w-full space-y-6 text-slate-900 pb-12">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-semibold text-slate-500">
              Pantau pemberitahuan pendaftaran murid baru, ulasan siswa, and persetujuan kurikulum
              ajar Anda.
            </p>
          </div>
        </div>
      ...
      {/* Main Content Area or Section Error */}
      {isQueryError ? (
        ...
        <Card className="border border-slate-200 shadow-sm bg-white overflow-hidden">
          <CardHeader className="pb-3 flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100">
            <div className="flex flex-wrap gap-1.5">
              {[
                { id: 'all', label: 'Semua' },
                { id: 'enrollment', label: 'Pendaftaran' },
                { id: 'review', label: 'Ulasan & Rating' },
                { id: 'publish', label: 'Publikasi Kelas' },
