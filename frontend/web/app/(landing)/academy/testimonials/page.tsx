      ...
      <section className="bg-primary py-12 text-white">
        <div className="container mx-auto px-4">
          ...
          <div className="max-w-2xl">
            <h1 className="text-3xl font-bold md:text-4xl">{ACADEMY_TESTIMONIALS.heading}</h1>
            <p className="mt-3 text-white/80 text-lg">{ACADEMY_TESTIMONIALS.subtext}</p>
          </div>
        </div>
      </section>
                ...
                >
                  ...
                  <div className="flex items-center gap-4 pt-6 border-t border-slate-50">
                    <div className="relative size-14 shrink-0 rounded-2xl overflow-hidden shadow-sm">
                      <Image src={item.avatar} alt={item.nama} fill className="object-cover" />
                    </div>
                    <div className="min-w-0">
                      ...
                      <div className="flex gap-0.5 mt-1">
                        {[...Array(5)].map((_, i) => (
                          <Star
                            key={i}
                            className={`size-3 ${i < item.rating ? 'fill-accent text-accent' : 'text-slate-200'}`}
                          />
                        ))}
                      </div>
                    </div>
                  ...
                  <div className="mt-6 flex items-center gap-2">
                    <Badge
                      variant="secondary"
                      className="bg-primary/5 text-primary border-none font-bold text-[10px] py-1 px-3 rounded-full flex items-center gap-1.5"
                    >
      ...
      <section className="container mx-auto px-4 mt-16">
        <div className="rounded-[2.5rem] bg-white p-8 md:p-12 text-center shadow-sm border border-slate-100">
          ...
          <p className="mt-4 text-gray-500 max-w-2xl mx-auto">
            Mulai perjalanan sukses Anda di dunia pertanian sekarang juga. Bergabunglah dengan
            ribuan alumni yang telah bertransformasi.
          </p>
          <div className="mt-8 flex justify-center">
            <Button
              asChild
              className="bg-primary hover:bg-primary-dark h-12 px-10 rounded-xl font-bold !text-white shadow-lg shadow-primary/20 transition-all active:scale-95"
            >
