      ...
      <section className="bg-primary py-12 text-white">
        <div className="container mx-auto px-4">
          ...
          <div className="max-w-2xl">
            <h1 className="text-3xl font-bold md:text-4xl">{ACADEMY_INSTRUCTORS.heading}</h1>
            <p className="mt-3 text-white/80 text-lg">{ACADEMY_INSTRUCTORS.subtext}</p>
          </div>
        </div>
      </section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {ACADEMY_INSTRUCTORS.items.map((item, idx) => (
            ...
            >
              ...
              <div className="flex flex-wrap justify-center gap-2 mb-8">
                {['Pakar Agronomi', 'Konsultan', 'Praktisi'].map((tag) => (
                  <Badge
                    key={tag}
                    variant="secondary"
                    className="bg-slate-50 text-slate-600 border-none font-bold text-[10px]"
                  >
                    {tag}
                  </Badge>
                ))}
              ...
              <Button
                asChild
                className="w-full bg-primary hover:bg-primary-dark font-bold rounded-xl h-12 !text-white shadow-lg shadow-primary/10"
              >
      ...
      <section className="container mx-auto px-4 mt-8">
        <div className="rounded-[2.5rem] bg-white p-8 md:p-12 text-center shadow-sm border border-slate-100">
          ...
          <p className="mt-4 text-gray-500 max-w-2xl mx-auto">
            Bagikan pengetahuan dan pengalaman Anda untuk memajukan pertanian Indonesia. Kami
            mencari pakar dan praktisi yang bersemangat mengajar.
          </p>
          <div className="mt-8 flex justify-center">
            ...
            >
              <Link href="/contact">Daftar sebagai Instruktur</Link>
            </Button>
          </div>
        </div>
      </section>
