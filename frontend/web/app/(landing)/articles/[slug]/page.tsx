...
import { ArrowLeft, Calendar, Clock, ChevronRight, Share2, BookmarkPlus } from 'lucide-react';
      ...
      <section className="relative min-h-[400px] md:min-h-[500px] lg:min-h-[600px] flex items-end overflow-hidden">
        ...
        <div className="absolute inset-0 z-0">
          {artikel.youtubeId ? (
            ...
            <Image src={artikel.image} alt={artikel.title} fill className="object-cover" priority />
          )}
          ...
          <div className="flex flex-col gap-12 lg:flex-row lg:gap-16">
              ...
              <div className="border-t border-slate-100 py-8">
                <div className="flex flex-wrap items-center gap-3">
                  ...
                  {['budidaya', 'pertanian', 'modern', 'smarttani'].map((tag) => (
                    <span
                      key={tag}
                      className="rounded-xl bg-slate-50 px-4 py-2 text-[10px] font-extrabold uppercase tracking-wider text-[#5d7a64] transition-all hover:bg-primary hover:text-white cursor-pointer border border-slate-100"
                    >
                      #{tag}
                    </span>
                  ))}
              ...
              <div className="rounded-3xl bg-slate-50 p-8 md:p-10 border border-slate-100 shadow-sm mt-8">
                <div className="flex flex-col items-center gap-6 sm:flex-row sm:items-start">
                  ...
                  <div className="text-center sm:text-left">
                    ...
                    <p className="text-sm font-medium text-[#5d7a64]">{artikel.penulis.gelar}</p>
                    <p className="mt-4 text-xs font-medium text-[#5d7a64] leading-relaxed max-w-lg">
                      Pakar berpengalaman yang berdedikasi untuk memajukan sektor pertanian
                      Indonesia melalui wawasan dan teknologi modern.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <aside className="w-full lg:w-[30%] lg:shrink-0">
              <div className="flex flex-col gap-12 lg:sticky lg:top-28">
                {/* Related Articles */}
                <div>
                  <h3 className="mb-8 text-lg font-extrabold text-[#17391f]">Artikel Terkait</h3>
                ...
                <div className="rounded-3xl bg-slate-50 p-8 border border-slate-100 shadow-sm">
                  <h3 className="mb-8 text-lg font-extrabold text-[#17391f]">Paling Populer</h3>
                  <div className="flex flex-col gap-6">
                    {ARTICLE_MOST_POPULAR.slice(0, 4).map((trending) => {
                      const matchedArticle = ARTICLE_ITEMS.find((a) =>
                        a.title.toLowerCase().includes(trending.title.toLowerCase().slice(0, 20))
                      );
